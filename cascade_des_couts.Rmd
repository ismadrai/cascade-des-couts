...
author:  |
  ...
  [<img src="images/logo.png">](https://legroupe.laposte.fr/)</div>
...
#calculons le poids de la charges qui est supprimé pour l'appliquer uniformément sur la base de charges
poids_formation<-prestation_formation/a$code_41
a<-a%>%
...
La clé taux de frais qui est calculé ci-haut pour la l'activité *ST - Transverses en DPOM-DCR* n'est pas bonne, cette clé doit être calculé sur la base de la répartition des charges après retraitement sur les activités pour lesquelles elle doit se ventiler. Le programme ci-dessous va corriger cette clé.
...
cle_tf<-cle_tf%>%
  left_join(dpom)%>%
  mutate(weight_code_36=cle_dpom)%>%
  select(-c(charges_dpom,cle_dpom))
...
#Répartition uniforme du poids de la prestation formation à enlever des charges
# On crée une nouvelle variable de charge après retraitement qui n'est modifié que sur le code_41
data_cascade<-data_cascade%>% 
  mutate(Ch_AP_Ret_prest_form=ifelse(Code_manuel==41,Ch_AP_Ret*(1-poids_formation),Ch_AP_Ret))

# Calcul de la ventilation sur les activités opérationnelles
data_cascade<-data_cascade%>% mutate_at(vars(starts_with("charge_code_")), funs(.*Ch_AP_Ret_prest_form))
#data_cascade<-data_cascade%>% mutate_at(vars(starts_with("charge_code_")), funs(.*Ch_AP_Ret))

rm(list=ls()[! ls() %in% c("data_cascade","cols","fldc","Part_CVAE","Part_CFE","file.name", "path.to.file","poids_formation")])
...
rm(list=ls()[! ls() %in% c("data_cascade","file.name", "path.to.file","poids_formation")])
...
#write.table(data_cascade,file="sorties/data.csv",sep=";",dec = ",",row.names = F)
